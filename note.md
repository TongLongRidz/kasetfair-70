...
| :--- | :--- | :--- | :---: |
| 1 | `/` (Landing Page) | หน้าหลักร้านค้า, เมนูสินค้า, ตะกร้าสินค้า | ✅ **เสร็จแล้ว** |
| 2 | `/order` | หน้ารถเข็น & กรอกข้อมูลสั่งซื้อ และชำระเงิน | ✅ **เสร็จแล้ว** |
| 3 | `/order/[uuid]` | **หน้าใบเสร็จรับเงิน & ติดตามสถานะออเดอร์**: ใบเสร็จ QR Code ตรวจสอบสถานะ, บันทึกใบเสร็จเป็นรูปภาพ (PNG), Auto-poll สถานะคำสั่งซื้อ Real-time | ✅ **เสร็จแล้ว** |
| 4 | `/payment/[uuid]` | หน้าแสดง QR Code พร้อมเพย์ และอัปโหลดสลิปหลักฐานโอนเงิน | ✅ **เสร็จแล้ว** |
| 5 | `/queue` | **หน้าจอแสดงสถานะคิวรวม (Queue Board)**: Fullscreen mode, แสดงคิวที่เรียก (Ready) ขนาดใหญ่ และคิวที่กำลังทำ (Preparing) แบบ FIFO เรียงตามเวลา | ✅ **เสร็จแล้ว** |
| 6 | `/about-us` | หน้าเกี่ยวกับร้านและช่องทางติดต่อ | ✅ **เสร็จแล้ว** |
...
| 7 | Management | `/admin/management/slip-check` | **ระบบตรวจสอบการชำระเงิน**: อนุมัติ/ปฏิเสธสลิปด้วย Dropdown และระบุเหตุผล, ตรวจสอบสลิปเทียบกับยอดเงิน, **ตั้งค่าระบบเปิด/ปิดการแนบรูปภาพสำหรับ Slip (`slip_upload_mode`: เปิด/ปิด) และ เงินสด (`cash_upload_mode`: เปิด/ปิด)** บันทึกลง Database ส่วนกลาง | ✅ **เสร็จแล้ว** |
| 8 | Management | `/admin/management/banner` | จัดการชุดแบนเนอร์และภาพประชาสัมพันธ์หน้าแรก | ⏳ Pending |
| 9 | POS | `/admin/pos/front-desk` | **หน้าจอขายหน้าร้าน (POS Walk-in)**: เลือกเมนู ปรับความหวาน ท็อปปิ้ง รถเข็นสินค้า | ✅ **เสร็จแล้ว** |
| 10 | POS | `/admin/pos/payment` | **หน้าจอชำระเงินหน้าร้าน (POS Payment)**: เงินสด/เงินทอน, พร้อมเพย์ QR, อัพโหลดสลิป, ออกบัตรคิว, Responsive จอคอมและจอมือถือ, เชื่อมต่อเงื่อนไข Slip Policy จากระบบ | ✅ **เสร็จแล้ว** |
| 11 | POS | `/admin/pos/kitchen` | **หน้าจอบาร์น้ำ/ห้องครัว (Kitchen Display)**: จัดการออเดอร์กำลังทำ, FIFO sorting, Fullscreen mode, KPI card แสดงเวลาออเดอร์ล่าสุด, Badge คอมโบและท็อปปิ้ง | ✅ **เสร็จแล้ว** |
| 12 | POS | `/admin/pos/queue` | **หน้าจอจัดการคิว & ส่งมอบสินค้า (Queue Management)**: สแกน QR Code รับเครื่องดื่มผ่านกล้อง/เครื่องสแกนเนอร์, กรอกหมายเลขคิว, Modal ยืนยันการส่งมอบ/เรียกคิว, รองรับ Fullscreen | ✅ **เสร็จแล้ว** |
...
- **ระบบตรวจสอบสลิปและอนุมัติออเดอร์ (Slip Verification):**
  ...
  - **ระบบตั้งค่าสวิตช์เงื่อนไขการอัปโหลดรูปภาพ (`slip_upload_mode` & `cash_upload_mode`):**
    - **พร้อมเพย์ QR:** เปิด/ปิด (`immediate` / `later`) กำหนดให้ลูกค้าระบบออนไลน์ หรือ POS ต้องอัปโหลดรูปสลิป
    - **เงินสด:** เปิด/ปิด (`immediate` / `later`) กำหนดให้แคชเชียร์ POS ต้องถ่ายรูป/อัปโหลดรูปหลักฐานการรับเงินสด
    - บันทึกการตั้งค่าลงฐานข้อมูล PostgreSQL (`system_settings`) และเชื่อมต่อไปยังทุกหน้า/ทุกเครื่องในระบบแบบ Real-time
  - **Modal ตรวจสอบหลักฐานการชำระเงิน:**
    - แสดงหมายเลขคิว, วิธีชำระเงิน, ยอดที่ต้องชำระ, รายการสั่งซื้อ และรูปภาพหลักฐาน (หรือปุ่มกดอัปโหลดรูปภาพย้อนหลัง)
    - เปลี่ยนสถานะผ่าน Dropdown Select: **ยังไม่ยืนยัน**, **ยืนยันแล้ว**, และ **เนียนเลยนะครับ**
    - เมื่อเลือก **เนียนเลยนะครับ** จะมีช่อง Textarea ให้ระบุเหตุผล/ข้อสงสัย พร้อมปุ่มกด **"บันทึก"** เพื่อยืนยันรายการ
...
| `slip_verification_status` | VARCHAR(20) | NOT NULL DEFAULT 'pending'; INDEX | สถานะตรวจสอบสลิป: `pending`, `verified`, `fraud` |
...
| `check_note` | TEXT | NULL | หมายเหตุการตรวจสอบสลิป/การชำระเงิน |
